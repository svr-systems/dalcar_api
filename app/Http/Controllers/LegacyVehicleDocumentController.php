...
use App\Models\LegacyVehicleDocument;
use DB;
use Illuminate\Http\Request;
use Throwable;

class LegacyVehicleDocumentController extends Controller {
  public function index(Request $req) {
    try {
      return $this->apiRsp(
        200,
        'Registros retornados correctamente',
        ['items' => LegacyVehicleDocument::getItems($req)]
      );
    } catch (Throwable $err) {
  ...
  public function show(Request $req, $id) {
    try {
      return $this->apiRsp(
        ...
        ['item' => LegacyVehicleDocument::getItem($req, $id)]
      );
    } catch (Throwable $err) {
      return $this->apiRsp(500, null, $err);
    }
  ...
  public function storeUpdate($req, $id) {
    ...
    try {
      ...
      $valid = LegacyVehicleDocument::valid($req->all());
      ...
      if ($store_mode) {
        $item = new LegacyVehicleDocument;
